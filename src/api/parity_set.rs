...
use super::types::{Address, ParityPeerType, H256};
...
impl<T: Transport> ParitySet<T> {
    ...
    pub fn execute_upgrade(&self) -> CallFuture<bool, T::Out> {
        CallFuture::new(
            self.transport().execute("parity_executeUpgrade", vec![]),
        )
    }
    ...
    pub fn hash_content(&self, url: &str) -> CallFuture<H256, T::Out> {
        ...
        CallFuture::new(
            self.transport().execute("parity_hashContent", vec![url]),
        )
    }

    /// Remove a reserved peer
    pub fn remove_reserved_peer(
        &self,
        enode: &str,
    ...
    pub fn set_author(&self, author: &Address) -> CallFuture<bool, T::Out> {
        ...
        CallFuture::new(
            self.transport().execute("parity_setAuthor", vec![address]),
        )
    }
    ...
    pub fn set_chain(&self, chain: &str) -> CallFuture<bool, T::Out> {
        ...
        CallFuture::new(
            self.transport().execute("parity_setChain", vec![chain]),
        )
    }
    ...
    pub fn set_extra_data(&self, data: &H256) -> CallFuture<bool, T::Out> {
        ...
        CallFuture::new(
            self.transport().execute("parity_setExtraData", vec![data]),
        )
    }

    /// Sets new gas ceiling target for mined blocks
    pub fn set_gas_ceil_target(
        &self,
        quantity: &H256,
    ) -> CallFuture<bool, T::Out> {
...
mod tests {
    ...
    use super::types::{
        Address, ParityPeerInfo, ParityPeerType, PeerNetworkInfo,
        PeerProtocolsInfo, H256,
    };
    use super::{Namespace, ParitySet};
    use rpc::Value;
